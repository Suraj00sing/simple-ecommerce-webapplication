...
public class updateProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String newPTitle= request.getParameter("newptitle");
            String newPDesc= request.getParameter("newpdesc");
            int newPPrice= Integer.parseInt(request.getParameter("newpprice"));
            Part newPImage= request.getPart("newpimage");
            
            Product product= new Product();
            product.setpName(newPTitle);
            product.setpDesc(newPDesc);
            product.setpPrice(newPPrice);
            product.setpPhoto(newPImage.getSubmittedFileName());
